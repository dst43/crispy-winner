...
def clip_grad_norm_(params, max_norm, aggregate_norm_fn=None) -> torch.Tensor:
    ...
    # if expertserver.utils.global_experts:
    #     expert_params = [expert.parameters() for expert in expertserver.utils.global_experts.expert_list]
    #     expert_params = [item for sulblist in expert_params for item in sulblist]
    #     expert_params = list(filter(grad_exists, expert_params))
    #     for p in expert_params:
    #         expert_grads.append(p.grad.detach())
            
    # print([ep.shape for ep in expert_grads])
    # print(f'{distributed_utils.get_global_rank()} {expert_grads[0].dtype} !!!expert grads: ', [torch.norm(ep, p=2, dtype=torch.float32) for ep in expert_grads])
    # assert False
