...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        for i in range(args.decoder_layers):
            ...
            if is_moe_layer:
                 expertserver.utils.global_layer_count_increment()
        ...
        if args.adaptive_softmax_cutoff is not None:
            ...
            # print('share_input_output_embed')#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        self.cpu_embedding_meter = AverageMeter('cpu_embedding_meter')
        self.gpu_embedding_meter = AverageMeter('gpu_embedding_meter')
        
        self.cpu_forward_meter = AverageMeter('cpu_forward_meter')
        self.gpu_forward_meter = AverageMeter('gpu_forward_meter')
    ...
    def forward(
        ...
        if not features_only:
            x = self.output_layer(x, extra)
        # print(f'{distributed_utils.get_global_rank()}, forward x: {torch.norm(x, p =2)}')
        return x, extra
    ...
    def extract_features_scriptable(
        ...
        # print('--------------------START-----------------')
        # print(prev_output_tokens.shape, prev_output_tokens)
        # embed tokens and positions
        
        def forward_embedding_wrapper(prev_output_tokens, token_embeddings, incremental_state):
            ...
            # print(f'prev_output_tokens: {prev_output_tokens.shape}, x: {x.shape}')
            cuda_end.record()
            cpu_end = time.time() * 1000
            # torch.cuda.synchronize()
            # self.cpu_embedding_meter.update((cpu_end - cpu_start))
            # self.gpu_embedding_meter.update(cuda_start.elapsed_time(cuda_end))
            return x

        x = forward_embedding_wrapper(prev_output_tokens, token_embeddings, incremental_state)
        ...
        def forward_wrapper(x):
            ...
            for idx, layer in enumerate(self.layers):
                ...
                x, layer_attn, moe_states, l_aux_i = layer(
                    x,
            ...
            cpu_end = time.time() * 1000
            # torch.cuda.synchronize()
            self.cpu_forward_meter.update((cpu_end - cpu_start))
            # self.gpu_forward_meter.update(cuda_start.elapsed_time(cuda_end))
            return x, attn, inner_states, l_aux, moe_states
        
        x, attn, inner_states, l_aux, moe_states = forward_wrapper(x)

        # print(f'project_out_dim: {self.project_out_dim}')
        if self.project_out_dim is not None:
            x = self.project_out_dim(x)
        # print(f'{distributed_utils.get_global_rank()} final x: {x.norm()}')
        return x, {"attn": [attn], "inner_states": inner_states, "l_aux": l_aux, "moe_states": moe_states}

    def output_layer(self, features, extra):
        """Project features to the vocabulary size."""
        if self.adaptive_softmax is None:
            # project back to size of vocabulary
            output = self.output_projection(features)
            moe_states = extra['moe_states']
            main_stream = torch.cuda.current_stream()
            if moe_states and not self.args.checkpoint_activations:
                def _backward_pre_expert_upload(*unused):
                    #print(f'moe_states: {moe_states}')
                    moe_state = moe_states.pop()
                    #print(f'backward projection experts pre-upload, moe_state: {moe_state}')
                    ...
                    #print('backward projection experts upload finished')
