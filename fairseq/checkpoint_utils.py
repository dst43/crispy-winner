...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    """
    # print(distributed_utils.get_global_rank(), 'load_checkpoint start', [torch.norm(param, p=2, dtype=torch.float32) for param in trainer.model.decoder.layers[0].moe_layer.experts.parameters()])
    ...
    # print(distributed_utils.get_global_rank(), 'load_checkpoint', [torch.norm(param, p=2, dtype=torch.float32) for param in trainer.model.decoder.layers[0].moe_layer.experts.parameters()])
    ...
    # print(distributed_utils.get_global_rank(), 'trainer load_checkpoint', [torch.norm(param, p=2, dtype=torch.float32) for param in trainer.model.decoder.layers[0].moe_layer.experts.parameters()])
    if (
