...
class _FP16OptimizerMixin(object):
    ...
    def backward(self, loss):
        ...
        # print(f'{distributed_utils.get_global_rank()} loss: {loss}')
        ...
        # fp32_params = []
        # for q in self.fp32_params:
        #     if hasattr(q, "expert"):
        #         fp32_params.append(q)
        # print([ep.shape for ep in expert_grads])
        # print(f'{distributed_utils.get_global_rank()} !!!fp32_params: ', [torch.norm(p, p=2, dtype=torch.float32) for p in fp32_params])
        # print(f'{distributed_utils.get_global_rank()} !!!fp16_params: ', [torch.norm(p, p=2) for p in params])
        # print(f'{distributed_utils.get_global_rank()} !!!fp16_grads: ', [torch.norm(ep, p=2) for ep in expert_grads])

    def _sync_fp16_grads_to_fp32(self):
        # print(self._needs_sync, self.has_flat_params)
        if self._needs_sync:
    ...
    def multiply_grads(self, c):
        ...
        # print(f'before mul_fac: {self._multiply_factor}')
        self._multiply_factor *= c
        # print(f'mul_fac: {self._multiply_factor}')
    ...
    def clip_grad_norm(self, max_norm, aggregate_norm_fn=None):
        ...
        # print([ep.shape for ep in expert_grads])
        # print(f'{distributed_utils.get_global_rank()} !!!fp32_grads: ', [torch.norm(ep, p=2) for ep in expert_grads])
        
        grad_norm = self._multiply_factor * self.fp32_optimizer.clip_grad_norm(
            0, aggregate_norm_fn
        )
        # print(f'!!!!!!!!!!!!!!!!!!! total_norm: {grad_norm}, {self._multiply_factor}')
        if self.scaler is not None:
...
class FP16Optimizer(_FP16OptimizerMixin, optim.FairseqOptimizer):
    ...
    def __init__(self, cfg: DictConfig, params, fp32_optimizer, fp32_params, **kwargs):
        ...
        if not getattr(cfg.common, "bf16", False):
            ...
            self.scaler = None
        expertserver.utils.set_global_scaler(self.scaler)
        assert expertserver.utils.global_scaler is self.scaler
