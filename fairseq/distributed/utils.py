...
def set_affinity(args):
    os.sched_setaffinity(os.getpid(), [args.device_id] + \
            [i for i in range(args.distributed_world_size, torch.multiprocessing.cpu_count())])
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    ...
    if torch.cuda.is_available() and not cfg.common.cpu and not cfg.common.tpu:
        ...
        set_affinity(cfg.distributed_training)
...
def get_all2all_group(moe_expert_count):
    if torch.distributed.is_initialized():
        if not hasattr(get_all2all_group, "_all2all_groups"):
            ...
            # print(f'all2allgroups: {all2all_groups}')
