...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self._model = model
        # print(distributed_utils.get_global_rank(), 'before half', [torch.norm(param, p=2) for param in self._model.decoder.layers[0].moe_layer.experts.parameters()])
        if not self.is_fsdp:
            if cfg.common.fp16:
        ...
        # print(distributed_utils.get_global_rank(), 'half', [torch.norm(param, p=2, dtype=torch.float32) for param in self._model.decoder.layers[0].moe_layer.experts.parameters()])
        if (
        ...
        # print(distributed_utils.get_global_rank(), 'to_cuda', [torch.norm(param, p=2, dtype=torch.float32) for param in self._model.decoder.layers[0].moe_layer.experts.parameters()])
    ...
    def get_train_iterator(
        ...
        # print(distributed_utils.get_global_rank(), 'get_train_iterator func start', [torch.norm(param, p=2, dtype=torch.float32) for param in self.model.decoder.layers[0].moe_layer.experts.parameters()])
        if load_dataset:
        ...
        # print(distributed_utils.get_global_rank(), 'load dataset', [torch.norm(param, p=2, dtype=torch.float32) for param in self.model.decoder.layers[0].moe_layer.experts.parameters()])
        ...
        # print(distributed_utils.get_global_rank(), 'get batch iterator', [torch.norm(param, p=2, dtype=torch.float32) for param in self.model.decoder.layers[0].moe_layer.experts.parameters()])
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        self._set_seed()
        # print(distributed_utils.get_global_rank(), 'before train', [torch.norm(param, p=2, dtype=torch.float32) for param in self.model.decoder.layers[0].moe_layer.experts.parameters()])
        self.model.train()
        self.criterion.train()
        self.zero_grad()
        # print(distributed_utils.get_global_rank(), 'in the train step', [torch.norm(param, p=2, dtype=torch.float32) for param in self.model.decoder.layers[0].moe_layer.experts.parameters()])
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            if self.is_moe and getattr(self.cfg.dataset, 'batch_size', None) is not None:
                try:
                    ...
                    print(f"fixed_src_seq_length: {fixed_src_seq_length}, sample: {sample['net_input']['src_tokens'].shape[1]}")
            ...
            try:
                with maybe_no_sync():
                    ...
                    # with torch.no_grad():
                    #     print(f'{distributed_utils.get_global_rank()} criterion', [param.norm() for param in self.criterion.parameters()])
        ...
        ## Remove it later when it is cheking the throghput
        with nvtx.annotate('sync stats'):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("multiply-grads"):
                ...
                # print(numer, (sample_size or 1.0))
                # print(f'!!before mul_grads: {self.optimizer._multiply_factor}')
                # print(numer, sample_size)
                self.optimizer.multiply_grads(numer / (sample_size or 1.0))
                # print(f'!!after mul_grads: {self.optimizer._multiply_factor}')
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                ...
                with nvtx.annotate('gpu optimize'):
    ...
    def lr_step_update(self):
        ...
        expertserver.utils.set_global_lr(new_lr)
